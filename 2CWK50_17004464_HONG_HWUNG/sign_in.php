...
if (isset($_SESSION['loggedInSkeleton']))
{
	...
	echo "<p class='mt-5'>You are already logged in, please log out first.</p>";
...
elseif (isset($_POST['username']))
{
	...
	if ($errors == "")
	{
		//check for a row in database with a matching username and password:
		$query = "SELECT * FROM users WHERE username = '$username' AND password = '$password'";
		
		//this query can return data
		$result = mysqli_query($connection, $query);
		
		//only 1 or 0 row return
		...
		if ($n>0) {
			...
			//set username into the session data
			$_SESSION['username'] = $username;
			//show sign in successful
				$message = "<p class='mt-5'>Hi, $username, you have successfully logged in, please <a href='account.php'>click here</a></p>";
			
		} else {
			//no matching data
			$show_signin_form = true;
			//show an unsuccessful msg
			$message = "<p class='mt-5'>Sign in failed, pls try again</p>";
		}
	...
	else
	{
		...
		$message = "<p class='mt-5'>Sign in failed, please check the errors shown above and try again</p>";
	}
}
else
{
